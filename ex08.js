...
function groupByAge(group) {
  const outputObject = {};
  for (let i = 0; i < group.length; i++) {
    if (outputObject.hasOwnProperty(group[i].age)) {
      outputObject[Number(group[i].age)].push(group[i]);
    } else {
      outputObject[Number(group[i].age)] = [group[i]];
    }
  }
  return outputObject;
}
